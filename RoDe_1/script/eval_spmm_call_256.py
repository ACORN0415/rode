...
dataset_base_dir = os.path.join(project_dir, "Baseline", "RoDe", "dataset")

# --- 입력 파일 자동 생성 로직 ---
input_csv_dir = os.path.join(project_dir, 'result', 'ref')
input_csv_path = os.path.join(input_csv_dir, 'baseline_h100_spmm_256.csv')
...
# --- 출력 파일 설정 ---
file_name = os.path.join(project_dir, 'result', 'Baseline', 'spmm', 'rode_spmm_f32_n256.csv')
head = ['dataSet', 'category', 'rows_','columns_','nonzeros_','sputnik','Sputnik_gflops','cusparse','cuSPARSE_gflops','rode','ours_gflops']
# ==================================================================
#               ★★★ 바로 이 부분이 핵심입니다 ★★★
# ==================================================================
...
for index, row in df.iterrows():
    ...
    if dataset_name in ['adaptive', 'delaunay_n22', 'rgg_n_2_22_s0']:
        print(f"Skipping filtered dataset: {dataset_name}")
        continue
    ...
    if found_path:
        ...
        shell_command = f"{project_dir}/Baseline/RoDe/build/eval/eval_spmm_f32_n32 {found_path} >> {file_name}"
